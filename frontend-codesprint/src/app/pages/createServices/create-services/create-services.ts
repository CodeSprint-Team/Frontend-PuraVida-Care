...
export class CreateServicesComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.serviceForm.invalid) {
      console.log('Formulario inválido:', this.serviceForm.errors);
      return;
    }
    ...
    const serviceData: ServiceRequest = {
      ...
      providerProfileId: this.providerId,
      serviceCategoryId: this.getCategoryId(this.f['category'].value),
      zone: this.f['zone'].value,
      modality: this.f['modality'].value,
      requirements: {
        experience: this.requirements['experience'].value,
        license: this.requirements['license'].value,
        certification: this.requirements['certification'].value
      }
    };

    console.log('Enviando al backend:', serviceData);
    ...
    this.serviceService.createService(serviceData).subscribe({
      ...
      error: (error: any) => {
        ...
        alert('Error al crear el servicio. Verifica los datos e intenta de nuevo.');
      }
    });
  }
